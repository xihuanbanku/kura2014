...
define("PAGE_COUNT", 500);
    ...
    function initPage($pageCount=100000, $pageIndex=1, $exportType = null) {
        ...
        $query = "select a.id, a.cp_number, a.cp_parent, a.cp_tm, a.cp_name, REPLACE(a.cp_title, ' ', ' ') cp_title, REPLACE(a.cp_detail,'～','~') cp_detail, a.cp_sale1, a.cp_jj,
            ...
            b.l_note,b.l_asin, a.cp_dtime, b.number, b.v_number, b.p_kid, b.package_size, b.kid, b.l_id, b.l_floor, b.l_shelf, b.l_zone, b.l_horizontal, b.l_vertical,
        ...
        if($exportType =="export") {
            $query .= ", group_concat(b.l_state1) gc_l_state1, group_concat(b.l_state2) gc_l_state2, group_concat(b.l_state3) gc_l_state3, group_concat(b.l_state4) gc_l_state4, group_concat(case b.l_state5 when 20 then 0 when 22 then 8 when 24 then 7 else b.l_state5 end) gc_l_state5,
                ...
                group_concat(b.l_asin) gc_l_asin, group_concat(b.l_note) gc_l_note, group_concat(b.number) gc_number, group_concat(b.v_number) gc_v_number, group_concat(b.p_kid) gc_p_kid, group_concat(b.package_size) gc_package_size,
                group_concat(b.kid) gc_kid, group_concat(b.l_id) gc_l_id, group_concat(concat(b.l_floor,'-', b.l_shelf,'-', b.l_zone, '-',b.l_horizontal,'-', b.l_vertical)) gc_pos";
    ...
    function out_excel() {
        switch ($_REQUEST["shop"]) {
            case "rakuten":
                ...
                $str1_pc = iconv("utf-8", "Shift_jis", '<table width="400" bgcolor="000000"  cellspacing="1" cellpadding="3">'
                ...
                $str1_smartphone = iconv("utf-8", "Shift_jis", '<table width="100%" bgcolor="000000"  cellspacing="1" cellpadding="3">'
                    . '<tr><th colspan="2" align="center" bgcolor="DCDCDC"><b><font color="000000">商品説明</font></b></th></tr>'
                        . '<tr><th align="left" bgcolor="DCDCDC" width="18%"><font size="2" color="000000">管理番号</font></th><td bgcolor="ffffff">'
                            . '<font size="2" color="000000">');
...
//                             $csv_data .= "1".COMMA;
                            ...
                            $str6 = '</table>';
                            //PC用商品説明文
                            $csv_data .= $str1_pc.$bean['cp_number'].$str2.$maker.$str3.strRepacre($bean['cp_detail']).$str4.strRepacre($bean['cp_gg']).$str5.$str6;
                            $csv_data .= COMMA;
                            //モバイル用商品説明文
                            $csv_data .= strRepacre($bean['cp_gg']).COMMA;
                            //スマートフォン用商品説明文
                            $csv_data .= $str1_smartphone.$bean['cp_number'].$str2.$maker.$str3.strRepacre($bean['cp_detail']).$str4.strRepacre($bean['cp_gg']).$str5.$str6;
...
//                 echo($excel_data);
            ...
            case "zone":
                ...
                $myfile = fopen($csv_file, "a") or die("Unable to open file!");
                $csv_data = iconv("utf-8", "Shift_jis", "商品名\t商品コード\tmainkc_id\t倉庫号\tゾーン\tバーコード\t在庫数\t仮想数\t更新用在庫\n");
                ...
                for ($i=1; $i<=100; $i++) {
                    ...
                    if($results) {
                        foreach ($results as $bean) {
                            ...
                            $csv_data .= $bean['v_number'].TAB;
                            $csv_data .= ($bean['v_number']+$bean['number']).TAB;
                ...
                for ($i=1; $i<=100; $i++) {
                    ...
                    if($results) {
                        foreach ($results as $bean) {
                            ...
                            // 突然想到一个问题，要加一个判定 ，在库数＜＝0的时候 ，不管仮想数、総数 一定要等于0,
                            // 否则网上会继续贩卖 已经没有在库的产品。这个样子会出现一个情况，就是货卖了，但是没有东西
                            if($columnsArray[$index++]) {
                                $csv_data .= '"'.($bean['gc_number'] <= 0 ? 0 : $bean['gc_v_number']).'"'.TAB;
                            }
                            if($columnsArray[$index++]) {
                                $csv_data .= '"'.($bean['gc_number'] <= 0 ? 0 : $bean['gc_v_number']+$bean['gc_number']).'"'.TAB;
                            }
                            if($columnsArray[$index++]) {
                                $csv_data .= '"'.$bean['gc_p_kid'].'"'.TAB;
                            }
                            if($columnsArray[$index++]) {
