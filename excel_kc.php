...
// if($action=='download'){
// 	   if($cp_categories_down=='')echo "<script>alert('小分類');history.go(-1);</script>";
          
//           if (!is_null($labid) and $labid != '') {
...
//                    $query="select *, cat.categories from #@__mainkc,#@__basic left join #@__categories cat on #@__basic.cp_categories_down = cat.id where  #@__basic.cp_number LIKE '%".$stext."%' and #@__basic.cp_categories='$cp_categories' and #@__basic.cp_categories_down='$cp_categories_down' and #@__mainkc.p_id=#@__basic.cp_number".$labstring;
//                } elseif ($cp_categories !='' and $cp_categories_down == '') {
//                    $query="select *, cat.categories from #@__mainkc,#@__basic left join #@__categories cat on #@__basic.cp_categories_down = cat.id where  #@__basic.cp_number LIKE '%".$stext."%' and #@__basic.cp_categories='$cp_categories' and #@__mainkc.p_id=#@__basic.cp_number".$labstring;
//                } else {
//                    $query="select *, cat.categories from #@__mainkc,#@__basic left join #@__categories cat on #@__basic.cp_categories_down = cat.id where  #@__basic.cp_number LIKE '%".$stext."%' and #@__mainkc.p_id=#@__basic.cp_number".$labstring;
//                }
//                break;
//            case "2":
...
// 		if($cp_categories !='' and $cp_categories_down != '') {
//                    $query="select *, cat.categories from #@__mainkc,#@__basic left join #@__categories cat on #@__basic.cp_categories_down = cat.id where  #@__basic.cp_tm LIKE '%".$stext."%' and #@__basic.cp_categories='$cp_categories' and #@__basic.cp_categories_down='$cp_categories_down' and #@__mainkc.p_id=#@__basic.cp_number".$labstring;
//                } elseif ($cp_categories !='' and $cp_categories_down == '') {
//                    $query="select *, cat.categories from #@__mainkc,#@__basic left join #@__categories cat on #@__basic.cp_categories_down = cat.id where  #@__basic.cp_tm LIKE '%".$stext."%' and #@__basic.cp_categories='$cp_categories' and #@__mainkc.p_id=#@__basic.cp_number".$labstring;
//                } else {
//                    $query="select *, cat.categories from #@__mainkc,#@__basic left join #@__categories cat on #@__basic.cp_categories_down = cat.id where  #@__basic.cp_tm LIKE '%".$stext."%' and #@__mainkc.p_id=#@__basic.cp_number".$labstring;
//                }
//                break;
//            case "3":
...
// 		if($cp_categories !='' and $cp_categories_down != '') {
//                    $query="select *, cat.categories from #@__mainkc,#@__basic left join #@__categories cat on #@__basic.cp_categories_down = cat.id where  #@__basic.cp_name LIKE '%".$stext."%' and #@__basic.cp_categories='$cp_categories' and #@__basic.cp_categories_down='$cp_categories_down' and #@__mainkc.p_id=#@__basic.cp_number".$labstring;
//                } elseif ($cp_categories !='' and $cp_categories_down == '') {
//                    $query="select *, cat.categories from #@__mainkc,#@__basic left join #@__categories cat on #@__basic.cp_categories_down = cat.id where  #@__basic.cp_name LIKE '%".$stext."%' and #@__basic.cp_categories='$cp_categories' and #@__mainkc.p_id=#@__basic.cp_number".$labstring;
//                } else {
//                    $query="select *, cat.categories from #@__mainkc,#@__basic left join #@__categories cat on #@__basic.cp_categories_down = cat.id where  #@__basic.cp_name LIKE '%".$stext."%' and #@__mainkc.p_id=#@__basic.cp_number".$labstring;
//                }
//                break;
//            case "4":
...
// 		if($cp_categories !='' and $cp_categories_down != '') {
//                    $query="select *, cat.categories from #@__mainkc,#@__basic left join #@__categories cat on #@__basic.cp_categories_down = cat.id where  #@__basic.cp_helpword LIKE '%".$stext."%' and #@__basic.cp_categories='$cp_categories' and #@__basic.cp_categories_down='$cp_categories_down' and #@__mainkc.p_id=#@__basic.cp_number".$labstring;
//                } elseif ($cp_categories !='' and $cp_categories_down == '') {
//                    $query="select *, cat.categories from #@__mainkc,#@__basic left join #@__categories cat on #@__basic.cp_categories_down = cat.id where  #@__basic.cp_helpword LIKE '%".$stext."%' and #@__basic.cp_categories='$cp_categories' and #@__mainkc.p_id=#@__basic.cp_number".$labstring;
//                } else {
//                    $query="select *, cat.categories from #@__mainkc,#@__basic left join #@__categories cat on #@__basic.cp_categories_down = cat.id where  #@__basic.cp_helpword LIKE '%".$stext."%' and #@__mainkc.p_id=#@__basic.cp_number".$labstring;
//                }
//                break;
//            }
// 	   }
// 	   else {
               $query = "select *, cat.categories from #@__mainkc,#@__basic left join #@__categories cat on #@__basic.cp_categories_down = cat.id where #@__mainkc.p_id=#@__basic.cp_number";
//           }
// 日付範囲
if ($cp_sdate != '' and $cp_edate != '' ) {
    $query .= " and DATE(dtime) >= cast('".$cp_sdate."' as date) and DATE(dtime) <= cast('".$cp_edate."' as date)";
} else if ($cp_sdate != '' and $cp_edate == '' ) {
    $query .= " and DATE(dtime) >= cast('".$cp_sdate."' as date)";
} else if ($cp_sdate == '' and $cp_edate != '' ) {
    $query .= " and DATE(dtime) <= cast('".$cp_edate."' as date)";
}
...
$query .= " and #@__mainkc.number > 0 order by cp_number";
...
if($shop == "rakuten") {
    //楽天用CSVファイル作成
    $csv_data = "コントロールカラム,商品管理番号（商品URL）,商品番号,全商品ディレクトリID,タグID,PC用キャッチコピー,モバイル用キャッチコピー,"
    ...
    $str1 = '<table width="700" bgcolor="000000"  cellspacing="1" cellpadding="3">'
    ...
    $str2 = '</font></td></tr><tr><th align="left" bgcolor="DCDCDC"><font size="2" color="000000">メーカー名</font></th>'
            . '<td bgcolor="ffffff"><font size="2" color="000000">';
    $str3 = '</font></td></tr><tr><th align="left" bgcolor="DCDCDC"><font size="2" color="000000">製品仕様</font></th>'
            . '<td bgcolor="ffffff"><font size="2" color="000000">';
    $str4 = '</font></td></tr><tr><th align="left" bgcolor="DCDCDC"><font size="2" color="000000">バッテリーの型番</font></th>'
            . '<td bgcolor="ffffff"><font size="2" color="000000">';
    $str5 = '</font></td></tr><tr><th align="left" bgcolor="DCDCDC"><font size="2" color="000000">パソコンの型番</font></th>'
            . '<td bgcolor="ffffff"><font size="2" color="000000">';
    $str6 = '</font></td></tr><tr><th align="left" bgcolor="DCDCDC"><font size="2" color="000000">弊社の安心ポイント</font></th>'
            . '<td bgcolor="ffffff"><font size="2" color="000000">'
            ...
            . '</font></td></tr></table>';
    $constBattery = '【対応バッテリーの品番】';
    $constComputer = '【対応パソコンの型番】';
    $constGoodcode = '商品コード';
    ...
    while($rs=$row->getArray()){
        for ($i=1; $i<=57; $i++) {
            switch($i){
                ...
                case "8":
                    //商品名
                    $csv_data .= strRepacreBrToSpace($rs['cp_title']).$comma;
                    break;
                case "9":
                    //販売価格
                ...
                case "27":
                    ...
                    $cat = $rs['cp_categories'];
                     if($cat!=13){
                         $str4 = '</font></td></tr><tr><th align="left" bgcolor="DCDCDC"><font size="2" color="000000">商品詳細</font></th>'
                                 . '<td bgcolor="ffffff"><font size="2" color="000000">';
                         $str5 = '</font></td></tr>';
                         $str6 = '</table>';
    ...
    //出力ファイル名の作成
    $csv_file = "楽天_Store_". date ( "YmdHis" ) .'.csv';
    //MIMEタイプの設定
    ...
    //Amazon用Excelファイル作成
    $excel_data = "TemplateType=Computers\tVersion=2013.1106\t上3行は Amazon.com 記入用です。上3行は変更または削除しないでください。\n";
    $excel_data .= "商品管理番号\t商品コード(JANコード等)\t商品コードのタイプ\t商品名\tブランド名\tメーカー名\t商品タイプ\tメーカー型番\t商品説明文\t"
    ...
    while($rs=$row->getArray()){
            ...
            for ($i=1; $i<=246; $i++) {
                switch($i){
                    ...
                    case "18":
                        //商品のコンディション説明
                        $excel_data .= "代引き、コンビニ受取&支払いなどにも対応しています。"
                    ...
                    case "52":
                        //商品説明の箇条書き1
                        $excel_data .= "・ご希望の商品と当方の商品イメージは一致することをご確認下さい。".$tab;
                        break;
                    case "53":
                        //商品説明の箇条書き2
                        $excel_data .= "・ご不明がございましたら、ご問い合わせ下さい。".$tab;
                        break;
                    case "54":
                        //商品説明の箇条書き3
                        $excel_data .= "・一般的にバルク品とは、メーカー保証書や説明書が付属されていない簡易包装の商品となります。".$tab;
                        break;
                    case "55":
                        //商品説明の箇条書き4
                        $excel_data .= "・日本国内発送".$tab;
                        break;
                    case "56":
                        //商品説明の箇条書き5
                        $excel_data .= "".$tab;
...
//    $csv_data = "商品名,商品コード,メーカ,市場価格,当店価格,ポイント購買額度,商品元画像,"
//    . "商品画像,商品縮略画像,キーワード,商品概要,商品詳細,商品重量,在庫数,在庫切れ警告数,逸品,新品,人気,販売,一般商品,実物商品\n";
        $csv_data = "商品名,商品コード,ゾーン,バーコード,在庫数\n";
    
    
    while($rs=$row->getArray()){
        // 商品名
        $csv_data .= $rs['cp_title'].$comma;
        // 商品コード
        $csv_data .= $rs['cp_number'].$comma;
        // ゾーン
        $csv_data .= $rs['l_floor'].'-'.$rs['l_shelf'].'-'.$rs['l_zone'].'-'.$rs['l_horizontal'].'-'.$rs['l_vertical'].$comma;
        // バーコード
        $csv_data .= $rs['cp_tm'].$comma;
        // 在庫数
        $csv_data .= $rs['number'].$comma;

//        // 市場価格
//        $market = $rs['cp_sale1']*1.2;
//        $csv_data .= $market.$comma;
//        // 当店価格
//        $csv_data .= $rs['cp_sale1'].$comma;
//        // ポイント購買額度
//        $csv_data .= "0".$comma;
//        // 商品元画像
//        $csv_data .= $rs['cp_url'].$comma;
//        // 商品画像
//        $csv_data .= $rs['cp_url'].$comma;
//        // 商品縮略画像
//        $csv_data .= $rs['cp_url'].$comma;
//        // キーワード
//        $keyword = get_name($rs['cp_categories'], categories);
//        $csv_data .= $keyword.$comma;
//        // 商品概要
//        $csv_data .= "".$comma;
//        // 商品詳細
//        $csv_data .= strRepacre($rs['cp_detail']).'<br>'.strRepacre($rs['cp_gg']).$comma;
//        // 商品重量
//        $csv_data .= "0".$comma;
//        // 在庫数
//        $csv_data .= $rs['number'].$comma;
...
//        $csv_data .= "0\n";
    ...
    //出力ファイル名の作成
    $csv_file = "ZONE_Store_". date ( "YmdHis" ) .'.csv';
    //MIMEタイプの設定
    header("Content-Type: application/octet-stream");
    //名前を付けて保存のダイアログボックスのファイル名の初期値
    header("Content-Disposition: attachment; filename={$csv_file}");
