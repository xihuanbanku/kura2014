...
idh=document.forms[0].i_dh.value;
number=document.forms[0].rk_number.value;
price=document.forms[0].back_price.value;
...
$query="select * from #@__kcbackgys where rdh='$r_dh'";
$bsql->SetQuery($query);
$bsql->Execute();
$rowcount=$bsql->GetTotalRow();
if ($rowcount==0){
 ShowMsg('引数エラー、または返品商品がありません。','-1');
 exit();
}
else{
 ...
 WriteNote(' 返品表 '.$r_dh.'を保存しました。',$logindate,$loginip,$username);
 ...
 $newsql->executenonequery("insert into #@__accounts(atype,amoney,abank,dtime,apeople,atext) values('返品払戻','".$money."','$thisbank','".$r_date."','".$r_people."','返品表番号：".$r_dh."')");
 $newsql->close();
 ShowMsg('返品表を変更しました。','system_gys_back.php');
$bsql->close();
exit();
    }
}
...
<table width="100%" border="0" id="table_style_all" cellpadding="0" cellspacing="0">
  ...
  <tr>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="2"> 
     <tr>
      <td><strong>&nbsp;購買返品管理</strong>&nbsp;&nbsp;- <a href="system_gys_back.php">新返品表</a></td>
     </tr>
     <tr>
      <td bgcolor="#FFFFFF">
<?php
	   ...
	   $printstring=" | <a href=report_b_gys.php?action=save&type=other&sday=".$row['r_dh'].">印刷</a>";
	   }
	   else{
	   $statusstring="<a href='?action=sure&t=yes&id=".$row['id']."'><img src='images/no.png' alt='審査' border='0'></a>";
	   $printstring="";
 ...
 $susql->close();
 showmsg('返品表審査状態を変更しました。','system_gys_back.php?action=seek');
 }
 else{
?>
...
<table width="100%" border="0" id="table_style_all" cellpadding="0" cellspacing="0">
  ...
  <tr>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="2">
     <tr>
      <td><strong><strong>&nbsp;仕入先へ返品管理</strong>(※：オレンジ色が必須項目)&nbsp;&nbsp;- <a href="system_gys_back.php">新返品表</a> - <a href="system_gys_back.php?action=seek">返品表一覧</a></td>
     </tr>
     <tr>
      <td bgcolor="#FFFFFF">
    ...
    <td class="cellcolor">&nbsp;<input type="text" name="r_dh" value="<?php echo $cdh; ?>" readonly class="rtext" size="15">&nbsp;(担当者：<input type="text" name="r_people" value="<?php echo Getcookie('VioomaUserID'); ?>" readonly class="rtext" size="8">時間：<input type="text" name="r_date" value="<?php echo GetDateTimeMk(time());?>"  readonly class="rtext">)</td>
  </tr>
  <tr>
    <td class="cellcolor" width="30%">商品検索情報：<br></td>
    <td>&nbsp;<input type="text" name="seek_text" value="商品選択" onclick="getinfo()">&nbsp;(クイック検索)
  ...
  <tr>
    ...
    <td>&nbsp;<input type="text" name="rk_number" size="5"><input type="text" class="rtext" name="showdw" readonly size="5">(返品数＞在庫数の場合、全部返品とする)
  ...
  <tr>
    ...
    <td>&nbsp;<input type="button" value=" 該当返品表に登録 " onclick="putrkinfo()">&nbsp;&nbsp;<input type="submit" value="保存"></td>
  </tr></form>
  <tr>
   <td colspan="2">
