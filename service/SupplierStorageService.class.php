...
session_start();
    ...
    function initPage($pageCount=100000, $pageIndex=1) {
        ...
        $query = "select * from jxc_supplier_storage a
            where del_flag=0
            and owner = {$userID}";
...
//         }
    ...
    function initPageCount() {
        $query = "select count(0) c, sum(cp_count) sc
        from jxc_supplier_storage
            where  del_flag=0";
//         if(!empty($_REQUEST["s_atime"])){
//            $s_atime = $_REQUEST["s_atime"];
//            $query .= " and atime >='{$s_atime}'";
...
//                         }
                        ...
                        $notesql="insert into `jxc_supplier_storage`(`cp_number`, `cp_name`, `cp_type`, `cp_price`, `cp_count`, `total`, `track_number`, `remark1`, send_status, status)
                    ...
                    case "u":
                        $index = 2;
                        $total = intval($strs[4])*intval($strs[5]);
                        $notesql="update `jxc_supplier_storage`
                        ...
                        break;
                    case "d":
                        $notesql="update `jxc_supplier_storage` set del_flag=1
                                     where cp_number = '".trim($strs[1])."'";
                        $importStat['d']++;
                        break;
                    default:
    ...
    function delete() {
        $userID = $_COOKIE['userID'];
        $id = $_REQUEST["id"];
        $query = "update jxc_supplier_storage set del_flag = 1 where id = ".$id ."
    ...
    function update() {
        ...
        $newsql = new ezSQL_mysql();
        $query = "update jxc_supplier_storage 
                set 
                cp_number     ='".$cp_number."',
                cp_name       ='".$cp_name."',
                cp_type       ='".$cp_type."',
