...
class receiveMail
{
	...
	function receiveMail($username,$password,$EmailAddress,$mailserver='localhost',$servertype='pop',$port='110',$ssl = false) //Constructure
	{
		...
		{
			$strConnect='{'.$mailserver.':'.$port. '/pop3'.($ssl ? "/ssl" : "").'/novalidate-cert}INBOX';
		}
...
// 		$this->marubox = imap_search($conn, 'SEEN');
...
// 					$file['size']      = $struckture->parts[$key]->dparameters[1]->value;
				...
				// 处理内容中包含图片的部分
				if(!isset($struckture->parts) && $struckture->parts[$key]->parts)
				{
...
// 			if (!isset($arr['host']) || !isset($arr['path']))continue;
	...
	function readMails($mid) // Delete That Mail
	{
		if(!$this->marubox)
			return false;
