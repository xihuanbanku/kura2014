...
function SpGetPinyin($str,$ishead=0,$isclose=1){
	...
	for($i=0;$i<$slen;$i++){
		...
		}else if( preg_match("[a-z0-9]",$str[$i]) ){	$restr .= $str[$i]; }
		else{ $restr .= "_";  }
	}
...
function SpCreateDir($spath,$siterefer="",$sitepath=""){
	...
	$flink = false;
	if($siterefer==1) $truepath = preg_replace("/{1,}","/",$cfg_basedir."/".$sitepath);
	else if($siterefer==2){
		$truepath = $sitepath;
		if($isSafeMode||$cfg_ftp_mkdir=='はい'){ echo "PHPセーフモードで、主サイト以外のディレクトリにファイル保存ができません。"; exit(); }
	}
	...
	foreach($spaths as $spath){
		...
		if(!is_dir($truepath) || !is_writeable($truepath)){
			 ...
			 if(!$isok){ echo "ディレクトリ作成、修正".$truepath." 失敗しました。<br>"; CloseFtp(); return false; }
		}
	}
	CloseFtp();
	return true;
...
function SpGetNewInfo(){
	...
	if( preg_match("[a-z\-]{1,}\.[a-z]{2,}",$nurl) ){ $nurl = urlencode($nurl); }
	else{ $nurl = "test"; }
