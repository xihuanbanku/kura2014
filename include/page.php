...
class DataList
{
	...
	function __construct()
 	{
 		...
 		if(empty($nowpage)||preg_match("[^0-9]",$nowpage)) $nowpage=1;
  ...
  function DataList(){
	...
	function GetPageList($list_len)
	{
		...
		if($list_len==""||preg_match("[^0-9]",$list_len)) $list_len=3;
		$totalpage = ceil($this->totalResult/$this->pageSize);
		
		if($totalpage<=1&&$this->totalResult>0) return "1ページ/".$this->totalResult."件"; 
		if($this->totalResult == 0) return "0ページ/".$this->totalResult."件"; 
