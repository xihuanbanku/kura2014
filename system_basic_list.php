...
function selectpro(value,id,gg,dw,s1,s2,s3,lid,number,lab,floor,shelf,zone,horizontal,vertical){
...
//window.opener.document.<?php echo $form ?>.sale.value=s3; 
window.opener.document.<?php echo $form ?>.labid.value=lid;
window.opener.document.<?php echo $form ?>.number.value=number;
window.opener.document.<?php echo $form ?>.labnm.value=lab;
window.opener.document.<?php echo $form ?>.positions.value=floor + "-" + shelf + "-" + zone + "-" + horizontal + "-" + vertical;
window.opener.document.<?php echo $form ?>.sale.value=s3;
//window.opener.sale_string.innerHTML="&nbsp;<input type='radio' name='sale1' checked value='"+s1+"' onclick='setsale(this.value)'>"+s1+"&nbsp;小売<input type='radio' name='sale1' value='"+s2+"' onclick='setsale(this.value)'>"+s2+"&nbsp;卸売<input type='radio' name='sale1' value='"+s3+"' onclick='setsale(this.value)'>"+s3+" 特殊";
window.close(); 
return false; 
}
</script>
...
{ 
  ...
  window.location.href="system_basic_list.php?form=<?php echo $form;?>&field=<?php echo $field;?>&sort=<?php echo $sort;?>&cp_categories="+locationid;
} 
function chgurl(cid){
cp_cate=document.forms[0].cp_categories.value;
if(cp_cate=='')
window.location.href="system_basic_list.php?form=<?php echo $form;?>&field=<?php echo $field;?>&sort=<?php echo $sort;?>&cp_categories=&cp_categories_down="+cid;
else
window.location.href="system_basic_list.php?form=<?php echo $form;?>&field=<?php echo $field;?>&sort=<?php echo $sort;?>&cp_categories="+cp_cate+"&cp_categories_down="+cid;
}
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="2">
     ...
     <tr>
      <td bgcolor="#FFFFFF">
	   ...
	   switch($sort){
	    case "1":
		$query="select * from #@__mainkc,#@__basic where #@__basic.cp_number like '%".$stext."%'".$catestring." and #@__mainkc.p_id=#@__basic.cp_number and #@__basic.cp_style=1 order by #@__mainkc.dtime desc, #@__mainkc.p_id";
		break;
            case "2":
		$query="select * from #@__mainkc,#@__basic where #@__basic.cp_tm like '%".$stext."%'".$catestring." and #@__mainkc.p_id=#@__basic.cp_number and #@__basic.cp_style=1 order by #@__mainkc.dtime desc, #@__mainkc.p_id";
		break;
            case "3":
		$query="select * from #@__mainkc,#@__basic where (#@__basic.cp_detail LIKE '%".$stext."%' or #@__basic.cp_title LIKE '%".$stext."%')".$catestring." and #@__mainkc.p_id=#@__basic.cp_number and #@__basic.cp_style=1 order by #@__mainkc.dtime desc, #@__mainkc.p_id";
		break;
            case "4":
		$query="select * from #@__mainkc,#@__basic where #@__basic.cp_helpword LIKE '%".$stext."%'".$catestring."and #@__mainkc.p_id=#@__basic.cp_number and #@__basic.cp_style=1 order by #@__mainkc.dtime desc, #@__mainkc.p_id";
		break;
		}
	   }
	   else
       $query="select * from #@__mainkc,#@__basic where #@__mainkc.p_id=#@__basic.cp_number and #@__basic.cp_style=1".$catestring." order by #@__mainkc.dtime desc, #@__mainkc.p_id";
...
$dlist->SetSource($query);
	   ...
	   <td><center><input type='checkbox' name='sel_pro".$row['id']."' value='".$row['cp_name']."' "
                   ...
                   . "".$row['cp_sale'].",".$row['cp_saleall'].",".$row['cp_sale1'].",".$row['l_id'].",".$row['number'].","
                   . "'".get_name($row['l_id'],'lab')."',"
                   . "'".$row['l_floor']."','".$row['l_shelf']."','".$row['l_zone']."','".$row['l_horizontal']."','"
                   . "".$row['l_vertical']."')\"></td>\r\n
	   </tr>";
	   }
