...
if($id=='')ShowMsg('不正操作','system_class.php');
...
ShowMsg('不正操作','-1');
 ...
 echo "<script language='javascript'>alert('大分類を削除する前に、該当大分類の小分類を削除してください。');history.go(-1);</script>";
 else{
 $msql->ExecuteNoneQuery("delete from #@__categories where id='$id'");
 WriteNote('大分類削除しました。'.$row['categories'],getdatetimemk(time()),getip(),$username);
 ShowMsg('大分類を削除しました。','system_class.php');
 }
 $msql->close();
 }
 else{
  $msql=New Dedesql(false);
  $msql->ExecuteNoneQuery("delete from #@__categories where id='$id'");
  WriteNote('小分類削除しました。'.$row['categories'],getdatetimemk(time()),getip(),$username);
  ShowMsg('小分類削除しました','system_class.php');
  $msql->close();
 }
 $dsql->close();
}
