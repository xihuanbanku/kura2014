...
session_start();
    ...
    function initPage($pageCount=100000, $pageIndex=1) {
        ...
        $query = "select * from jxc_finance_reloadflow a
            where del_flag=0";
    ...
    function initPageCount() {
        $query = "select count(0) c, sum(cp_count) sc, sum(total) st
        from jxc_finance_reloadflow
            where  del_flag=0";
        if(!empty($_REQUEST["s_atime"])){
           $s_atime = $_REQUEST["s_atime"];
           $query .= " and atime >='{$s_atime}'";
...
//                             break 2;
//                         }
                        $total = floatval($strs[4])*intval($strs[5]);
                        $notesql="insert into `jxc_finance_reloadflow`(`cp_number`, `cp_name`, `cp_type`, `cp_price`, `cp_count`, `total`, `track_number`, `company`)
                    ...
                    case "u":
                        $index = 2;
                        $total = floatval($strs[4])*intval($strs[5]);
                        $notesql="update `jxc_finance_reloadflow`
                            set `cp_name` = '".trim($strs[$index++])."',
                                ...
                                `company` = '".trim($strs[$index++])."'
                                     where cp_number = '".trim($strs[1])."'";
                        $importStat['u']++;
                        break;
                    case "d":
                        $notesql="update `jxc_finance_reloadflow` set del_flag=1
                                     where cp_number = '".trim($strs[1])."'";
                        $importStat['d']++;
                        break;
                    default:
    ...
    function insert() {
        ...
        //总金额 =单价*数量
        $total    =floatval($cp_price)*intval($cp_count);
        
        $newsql = new ezSQL_mysql();
        $query = "INSERT INTO `jxc_finance_reloadflow` (`cp_number`, `cp_name`, `cp_type`, `cp_price`, `cp_count`, `total`, `track_number`, `company`, `picture_name`)
            values('{$cp_number}', '{$cp_name}', '{$cp_type}', '{$cp_price}', '{$cp_count}', '{$total}', 
            '{$track_number}', '{$company}','{$picture_name}')";
    ...
    function delete() {
        ...
        $id = $_REQUEST["id"];
        $query = "update jxc_finance_reloadflow set del_flag = 1 where id = ".$id ."
    ...
    function update() {
        ...
        $newsql = new ezSQL_mysql();
        $query = "update jxc_finance_reloadflow 
                set 
                cp_number     ='".$cp_number."',
                cp_name       ='".$cp_name."',
                cp_type       ='".$cp_type."',
    ...
    function exportExcel() {
        $title = "上货流水";
		$headers = array('管理番号','商品管理番号','商品名称','规格','单价','数量','总金额','追踪番号','运送公司','登记日期','凭证');
        $results = initPage();
