...
if ($action=='save'){
    ...
    //复制需要先判断提交的商品code和bar_code是否已存在
    $squery="select * from #@__basic where cp_number='$cp_number' or cp_tm = '$cp_tm'";
    $bsql->SetQuery($squery);
    $bsql->Execute();
    $rowcount=$bsql->gettotalrow();
    if($rowcount>0){
        Showmsg('[商品コード] 或  [バーコード]已存在','-1');
        exit();
    }
    
    $cp_title = str_replace(array("\r\n","\n","\r"), ' ', $cp_title);
    $query="insert into #@__basic (`cp_number`, `cp_parent`, `cp_tm`, `cp_name`, `cp_title`, `cp_detail`, 
    `cp_gg`, `cp_categories`, `cp_categories_down`, `cp_dwname`, `cp_style`, `cp_jj`, `cp_sale`, `cp_saleall`, `cp_sale1`, 
    `cp_sdate`, `cp_edate`, `cp_gys`, `cp_helpword`, `cp_helpword_1`, `cp_helpword_2`, `cp_helpword_3`, `cp_helpword_4`, `cp_helpword_5`, 
    `cp_helpword_6`, `cp_helpword_7`, `cp_helpword_8`, `cp_helpword_9`, `cp_bz`, 
    `cp_url`, `cp_url_1`, `cp_url_2`, `cp_url_3`, `cp_url_4`, `cp_browse_node_1`, `cp_browse_node_2`, 
    `cp_bullet_1`, `cp_bullet_2`, `cp_bullet_3`, `cp_bullet_4`, `cp_bullet_5`, `cp_bullet_6`) VALUES 
    ('".$cp_number."', '".$cp_parent."','".$cp_tm."', '".$cp_name."', '".$cp_title."',". "'".$cp_detail."', 
    '".$cp_gg."', '".$cp_categories."', '".$cp_categories_down."', '".$cp_dwname."', '".$cp_style."', '".$cp_jj."', '".$cp_sale."', '".$cp_saleall."','".$cp_sale1."',
    '".$cp_sdate."','".$cp_edate."','".$cp_gys."','".$cp_helpword."','".$cp_helpword_1."','".$cp_helpword_2."','".$cp_helpword_3."','".$cp_helpword_4."','".$cp_helpword_5."',
        '".$cp_helpword_6."','".$cp_helpword_7."','".$cp_helpword_8."','".$cp_helpword_9."','".$cp_bz."',
    '".$cp_url."','".$cp_url_1."','".$cp_url_2."','".$cp_url_3."','".$cp_url_4."','".$cp_browse_node_1."','".$cp_browse_node_2."',
    '".$cp_bullet_1."','".$cp_bullet_2."','".$cp_bullet_3."','".$cp_bullet_4."','".$cp_bullet_5."','".$cp_bullet_6."')";
    ...
    showmsg('商品情報をcopyしました。','system_basic_cp.php?action=seek');
     $loginip=getip();
     $logindate=getdatetimemk(time());
     $username=GetCookie('VioomaUserID');
     WriteNote('商品'.$cp_number.' の情報をcopyしました。',$logindate,$loginip,$username);
    $bsql->close();
    exit();
}
...
<table width="100%" border="0" id="table_style_all" cellpadding="0" cellspacing="0">
 ...
 <form action="system_basic_copy.php?action=save" method="post" name="form1">
   ...
   <tr>
    ...
    <td>&nbsp;<input type="text" name="cp_number" value="<?php echo $row['cp_number'] ?>"/></td>
  </tr>
  <tr>
    <td class="cellcolor" width="30%">親子関連：</td>
    <td>&nbsp;<input type="text" name="cp_parent" value="<?php echo $row['cp_parent'] ?>" /></td>
  </tr>
  <tr>
    <td class="cellcolor" width="30%">バーコード：</td>
    <td>&nbsp;<input type="text" name="cp_tm" value="<?php echo $row['cp_tm'] ?>"/></td>
  </tr>
  <tr>
...
//         }
  ...
  <tr>
    ...
    <td>&nbsp;<input type="button" value="提交" onclick="checkForm()"></td>
  </tr></form>
</table>
    </td>
     </tr>
