...
$(function(){
	$("#subButton").click(function(){
        ...
        var count=0;
        $.each($("#formTr input:gt(0):lt(4)"), function(i, item) {
    	...
    	$.ajax({
    		type: "post",
    		url: "../service/FinanceFeeService.class.php",
		...
		success: function(data){
    		...
    		$.each(data.results, function(entryIndex, entry){
        		...
        		html +="</tr>"
            		...
            		+"    <td><input type=\"hidden\" value=\""+entry.owner+"\"/><select style=\"width:70px;\" name=\"owner\">" + ownerOptions +"</select> </td>"
    	    ...
    	    if(e_atime == "") {
    	    	e_atime = "<?php echo date("Y-m-d")?>";
  	   	    ...
  	   	    $("#totalPage").html("<span style='font-size: 100%;color:#A17B20'>总共："+recordCount+"条, 实际收入：["
  	    	   	    ...
  	    	   	    +"] 应付：["+number_format(data.totalcount.s_real_outgoing)+"] 实际结余：["+number_format(data.totalcount.s_real_rest)+"] 挂账结余：["+number_format(data.totalcount.s_rest_not_back)+"] 时间区间:["+s_atime+"至"+e_atime+"]</span>");
		}
	});
}
...
function exportExcel() {
	...
	var url = "../service/FinanceFeeService.class.php?"+$("select").serialize()+"&nocache="+new Date().getTime()+"&"+param+"&flag=exportExcel";
    window.open(url);
}
//全选
...
function updateStatus() {
    ...
    $.ajax({
		type: "post",
		url: "../service/FinanceFeeService.class.php?" + param,
		data: {
			"flag":"updateStatus"},
		success: function(data){
			if(data >0) {
...
function show_window() {
	...
	$("#contentTable tr:gt(0) input[type=checkbox]:checked").each(function(i, item){
		html+=""
			...
			+"    <td>" + $(item).parent().nextAll(":eq(1)").text() + "</td>"
...
<?php
require_once '../service/FinanceFeeService.class.php';
		...
		<tr>
			...
			<td>
				<table width="100%" border="0" cellpadding="0" cellspacing="2">
					<tr>
						<td><strong>小额费用管理</strong>
    						<form name="form2"  method="post" enctype="multipart/form-data" >
    							上传内容:<input type="file" name="inputExcelBuy" id="inputExcelBuy"/><input type="submit" value="取込 "/>
        						<a href="../upload/Fee_Template.xlsx" style="font-size: 16px; color:#0000FF">标准模板</a>
    						</form>
						</td>
					</tr>
					<tr>
							...
							<table id="contentTable" width="100%" border="0">
								...
								<tbody>
                                    <tr id="formTr">
                                        ...
                                        <td><input size="10" name="limit_date" type="text" class="Wdate" onclick="WdatePicker()"/></td>
