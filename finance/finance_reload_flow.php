...
$(function(){
	$("#subButton").click(function(){
    	$.ajax({
    		type: "post",
    		url: "../service/FinanceReloadFlowService.class.php",
		...
		success: function(data){
    		...
    		$.each(data.results, function(entryIndex, entry){
        		html += ""
            		...
            		+"    <td style='text-align: right;'>"+number_format(entry.total)+"</td>"
            		+"    <td title=\""+entry.track_number+"\">"+entry.track_number+"</td>"
            		+"    <td title=\""+entry.company+"\">"+entry.company+"</td>"
        		...
        		html +="</tr>"
            		...
            		+"    <td><input value=\""+entry.company+"\" size=\"10\" name=\"company\" type=\"text\"/></td>"
            		+"    <td>"+entry.atime+"</td>"
    	    ...
    	    if(e_atime == "") {
    	    	e_atime = "<?php echo date("Y-m-d")?>";
  	   	    ...
  	   	    $("#totalPage").html("<span style='font-size: 100%;color:#A17B20'>总共："+recordCount+"条, 总数量：["
  	    	   	    +data.totalcount.sc+"] 总金额：["+number_format(data.totalcount.st)+"] 时间区间:["+s_atime+"至"+e_atime+"]</span>");
		}
	});
}
...
function exportExcel() {
	...
	var url = "../service/FinanceReloadFlowService.class.php?"+$("select").serialize()+"&nocache="+new Date().getTime()+"&"+param+"&flag=exportExcel";
    window.open(url);
}
//全选
...
function deleteChked() {
    ...
    $.ajax({
		type: "post",
		url: "../service/FinanceReloadFlowService.class.php?" + param,
		data: {
			"flag":"deleteChked"},
		success: function(data){
			if(data >0) {
...
<?php
require_once '../service/FinanceReloadFlowService.class.php';
		...
		<tr>
			...
			<td>
				<table width="100%" border="0" cellpadding="0" cellspacing="2">
					<tr>
						<td><strong>上货流水</strong>
    						<form name="form2"  method="post" enctype="multipart/form-data" >
    							上传内容:<input type="file" name="inputExcelBuy" id="inputExcelBuy"/><input type="submit" value="取込 "/>
        						<a href="../upload/Reload_Template.xlsx" style="font-size: 16px; color:#0000FF">标准模板</a>
    						</form>
						</td>
					</tr>
					<tr>
							...
							<table id="contentTable" width="100%" border="0">
								<thead>
									<tr>
										...
										<th class="cellcolor">规格</th>
										<th class="cellcolor">单价</th>
										<th class="cellcolor">数量</th>
										<th class="cellcolor">总金额</th>
										<th class="cellcolor">追踪番号</th>
										<th class="cellcolor">运送公司</th>
