...
<head>
<title><?php echo $cfg_softname;?>公告</title>
...
function initPage(pageIndex) {
	...
	var pageCount = 20;
	var recordCount = 0;
	var url = "../service/BulletinService.class.php?pageCount="+pageCount+"&pageIndex="+(pageIndex+1)+"&nocache="+new Date().getTime();
	var param = $("input").serialize();
	$.ajax({
		type: "post",
		url: url+"&"+param+"&"+$("select").serialize(),
		data: {"flag":"init"},
...
//     		alert(data);
			...
			if(data.totalcount != 0) {
        		$.each(data.results, function(entryIndex, entry){
        			...
        			html+="  <td><a href=\"javascript:void(0);\" onclick=\"openContent(" + entry.id + ", '" + entry.passwd + "')\"><b>" + entry.subject + "</b></a></td>"
...
function chgRemindMe(){
	...
	$.ajax({
		type: "post",
		url: url,
		data: {"flag":"chgRemindMe", 
			"id":$("#bulletin_id").val(), 
			"remind_me":$("#remind_me").val()},
		success: function(msg){
			if(msg > 0) {
				alert("设置提醒成功");
			} else {
	        ...
	        <td align="right">
    	        <div id="searchDiv">
					<select name="is_public">
						...
						<option value="2"/>紧急提示</option>
						<option value="3"/>お問い合わせ管理</option>
					</select>
    	                                发送时间:
