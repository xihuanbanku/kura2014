	...
	<form action="regsiter.php?action=sure" method="post">
	 ...
	 <tr bgcolor="#FFFFFF">
	  ...
	  if ($action=='sure'){
	   ...
	   $row=$sql->GetOne();
	   if ($rkey!=$row['key']){
	   echo "<script language='javascript'>alert('你的注册码不符合标准，请确认!咨询QQ：475994986');history.go(-1);</script>";
	   exit();
	   }
	   if($rfile==''){
...
Web进销存（PHP版）适应中小企业及个体用户使用，功能实用，操作简单，是您自动管理“进、销及存”不可缺少的帮手！ */
...
Web进销存（PHP版）适应中小企业及个体用户使用，功能实用，操作简单，是您自动管理“进、销及存”不可缺少的帮手！ */";
	 ...
	 <tr>
	  <td bgcolor="#EFEFEF">
	  Web进销存注册码：<input type="text" name="rkey" size="20" value="<?php echo $row['key'];?>" onclick="this.select()" readonly>
	  </td>
	 </tr>
	 <?php
	   $password='userma';
